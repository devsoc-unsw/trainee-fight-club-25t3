            ...
            <tbody className="divide-y divide-border">
              {data.map((t) => (
                <tr key={t.id} className="hover:bg-muted/50 transition-colors">
                  <td className="p-4 text-muted-foreground whitespace-nowrap">{t.date}</td>
                  <td className="p-4 font-medium">{t.description}</td>
                  <td className="p-4">
                    {t.category ? (
                       <span className="bg-secondary px-2 py-1 rounded text-xs border border-border">{t.category}</span>
                    ) : (
                       <span className="bg-muted text-muted-foreground px-2 py-1 rounded text-xs italic">Uncategorized</span>
                    )}
                  </td>
                  <td className="p-4 text-right text-red-500/80 font-mono">{t.debit ? fmtMoney(t.debit) : "-"}</td>
                  <td className="p-4 text-right text-green-500/80 font-mono">{t.credit ? fmtMoney(t.credit) : "-"}</td>
                  <td className="p-4 text-right text-muted-foreground font-mono">{t.balance ? fmtMoney(t.balance) : "-"}</td>
                  
                  <td className="p-4 text-right space-x-2">
                    <Button variant="ghost" size="icon" onClick={() => setEditItem(t)} className="h-8 w-8 text-blue-500 hover:text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/20">
                        <Pencil className="w-4 h-4"/>
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => setDeleteId(t.id)} className="h-8 w-8 text-red-500 hover:text-red-600 hover:bg-red-100 dark:hover:bg-red-900/20">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input 
                  ...
                  onChange={e => setEditItem({...editItem, balance: parseFloat(e.target.value)})} 
                />
              </div>
            </div>

            <div className="text-sm text-muted-foreground text-right mt-2">
                Net Amount: <span className={editItem?.amount < 0 ? "text-red-500 font-medium" : "text-green-500 font-medium"}>
